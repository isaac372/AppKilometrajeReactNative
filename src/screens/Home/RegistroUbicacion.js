...
const RegistroUbicacion = ({navigation, route}) => {
  ...
  const IdRela = CompañiaUsuario
    ...
    : 0;
  const type = tipo === 1 ? 'iniciacion  de ruta ' : 'Finalizacion  de ruta ';
  const datetype = moment(new Date()).format('LLL');
  const [Simage, setimage] = React.useState('');
  const [data, setdata] = React.useState({
    Kilometraje: '',
    Comentario: type + ' ' + datetype,
    Latitud: 0,
    Longitud: 0,
    IdRelacion: IdRela,
    AuxBase64Image: '',
  ...
  const CrearRegistro = () => {
    ...
    const getLiveLocation = async () => {
      ...
      if (locPermissionDenied) {
        ...
        dispatch(
          CreateRegisterUbicacion(data, navigation, latitude, longitude, tipo),
        );
      }
    };
    getLiveLocation();
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        {stateRegister.loading ? (
          ...
          <FAB
            style={styles.fab}
            label={tipo === 1 ? 'Iniciar' : 'Finalizar'}
